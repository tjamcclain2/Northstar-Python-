...
run once per process/invocation, use devin_optimized.py's number instead --
that one is the honest cost for that usage pattern.
...
def source_vector_from_angles(angle_grid) :
    ...
    """
        ...
        -- and returns a 1D CuPy array representing the unit-length vector from the center the Earth to the source/detector. Note that only the first two angles are actually needed to compute the unit vector.
...
def generate_model_detector_responses(
    ...
    weights_and_amps : list [weighted_H, weighted_L, amplitude_grid]
        ...
        The full response for detector d at (angle, amp, time) is
        weighted_{H,L}[angle, time, :] dot amplitude_grid[amp, :]. This used to
        be computed here via cp.einsum(...) into a materialized
        (n_ang, n_amp, n_times, 2) responses array, but that array is only ever
        consumed by get_best_fit_angles_deltas's fused abs_diff_sum kernel,
        which reconstructs it on the fly from these smaller ingredients
        instead (see module docstring, "FUSED REDUCTION KERNEL"). Returning
        the ingredients directly skips a ~2.6 GB allocation and the two einsum
        passes that used to fill it (at 500 angles x 500 amplitude combos).
    ...
    # returned as-is; get_best_fit_angles_deltas's fused kernel takes it from
    # here. (Previously: cp.einsum('atm,pm->apt', weighted_H, amplitude_grid)
    # and cp.einsum('atm,pm->apt', weighted_L, amplitude_grid) were written
    # into a cp.empty((n_ang, n_amp, n_times, 2)) responses array right here.)
...
def get_best_fit_angles_deltas(real_detector_responses, real_angles_array,
    ...
    Parameters:
        ...
        model_detector_responses (list): [weighted_H, weighted_L, amplitude_grid],
            ...
            (see that function's docstring for shapes/rationale). Unpacked and
            reshaped below into per-GW-mode components (wh0..wh3, wl0..wl3,
            a0..a3) so cp.ReductionKernel's automatic broadcasting reconstructs
            the (n_ang, n_amp, n_times) contraction on the fly for the fused
            abs_diff_sum kernel.
    ...
    Returns:
        list: [deltas, single_best_fit, weighted_best_fit]
            deltas (list): [sum_real_minimum_angle_deltas,
                ...
                sum_real_maximum_weighted_response_angle_deltas] -- the three
                angle deltas themselves (each a 0-d cp.ndarray), already computed.
...
def _run_pipeline_once(
    ...
    exactly once, with no timing and no printing beyond what the called
    functions already print (the compare step, get_best_fit_angles_deltas,
...
def run_northstar_pipeline(
    ...
    # Real random draws for the reported result start fresh here, so the
    # warmup pass above (whatever it drew) has no effect on reproducibility.
    start = cp.cuda.Event()
    end = cp.cuda.Event()
    start.record()
