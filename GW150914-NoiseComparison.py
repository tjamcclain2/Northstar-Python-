# -*- coding: utf-8 -*-
"""
Created on Mon Jul  7 10:41:21 2025
...
# ------------------------------------------------------
# 1️⃣ Load both H1 + L1, same setup
# ------------------------------------------------------
...
# ------------------------------------------------------
# 3️⃣ Find index of time bin: 2 s before merger
# ------------------------------------------------------

merger_offset = gps - (int(gps) - 4)  # relative to chunk start
target_offset = merger_offset - 2.0

print(f"Merger offset: {merger_offset:.3f} s")
print(f"Chosen bin: ~{target_offset:.3f} s (2 s before merger)")

idx_H1 = np.argmin(np.abs(t_H1 - target_offset))
idx_L1 = np.argmin(np.abs(t_L1 - target_offset))

print(f"H1 bin center at t = {t_H1[idx_H1]:.5f} s")
print(f"L1 bin center at t = {t_L1[idx_L1]:.5f} s")

# ------------------------------------------------------
# 4️⃣ Integrate total power for both
# ------------------------------------------------------

P_H1 = np.sum(Sxx_H1[:, idx_H1])
P_L1 = np.sum(Sxx_L1[:, idx_L1])

print(f"H1: Total power @ t = {t_H1[idx_H1]:.3f} s → {P_H1:.3e}")
print(f"L1: Total power @ t = {t_L1[idx_L1]:.3f} s → {P_L1:.3e}")
...
if P_H1 > P_L1:
    print("H1 stronger at this slice.")
else:
    print("L1 stronger at this slice.")
