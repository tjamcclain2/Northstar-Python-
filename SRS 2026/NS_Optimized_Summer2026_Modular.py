...
def generate_model_angles_array(number_angular_samples) :
    ...
    np.savetxt('angle_grid.csv', angle_grid, delimiter=',', fmt='%6f')
...
def generate_model_amplitudes_array(number_amplitude_combinations, gw_max_amps) :
    ...
    """
    amplitude_array = np.random.rand(number_amplitude_combinations, NUMBER_GW_MODES) * gw_max_amps

    np.savetxt('amplitude_array.csv', amplitude_array, delimiter=',', fmt='%6f')
...
def generate_model_detector_responses(
    ...
    # Generate or read model amplitude and angle grids
    if generate_new_random_values: generate_model_amplitudes_array(n_amp, gw_max_amps) # Shape: (n_amp, 4)
    amplitude_grid = np.loadtxt('amplitude_grid.csv', delimiter=',')
    if generate_new_random_values: generate_model_angles_array(n_ang) # Shape: (n_ang, 3)
    angle_grid = np.loadtxt('angle_grid.csv', delimiter=',')                    
...
def generate_hanford_noise_array(max_noise_amp,number_time_samples) :
    ...
    np.savetxt('hanford_noise_array.csv', noise_array, delimiter=',', fmt='%6f')

def generate_livingston_noise_array(max_noise_amp,number_time_samples) :

    noise_array = np.random.rand(number_time_samples)*max_noise_amp
    
    np.savetxt('livingston_noise_array.csv', noise_array, delimiter=',', fmt='%6f')
#=========================================================================
...
def generate_real_detector_responses(signal_frequency, signal_lifetime, detector_sampling_rate,
    ...
    # 5. Noise arrays
    if generate_new_random_values: generate_hanford_noise_array(max_noise_amp, number_time_samples)
    noise_h = np.loadtxt('hanford_noise_array.csv', delimiter=',')
    if generate_new_random_values: generate_livingston_noise_array(max_noise_amp, number_time_samples) 
    noise_l = np.loadtxt('livingston_noise_array.csv', delimiter=',')
...
def run_northstar_pipeline(
    ...
    generate_new_random_values=True # Set to False to use current random values (if they exist; check for appropriate .csv files first!)
):
    ...
    # Generate array of mapping data: sky angles from model_angles, 
    # summed_weights from the output of best_fit_data()

    mapping_data = np.column_stack((model_angles,best_fit_data[5]))

    # Create human-readable timestamped filenames
    timestamp = time.strftime("%d_%b_%Y_%H-%M-%S")
    output_filename = f"OPTIMIZED_northstar_output_{timestamp}.txt"
    mapping_data_filename = f"mapping_data_{timestamp}.txt"
    ...
    np.savetxt(mapping_data_filename, mapping_data, delimiter=",", fmt="%.6f", 
               header="Declination, Right Ascension, Polarization, Weighted Sum")

    print(f"\n[✔] Output also written to: {output_filename}")
    print(f"Data for sky mapping written to: {mapping_data_filename}")
    print(f"Both files can be found in {current_working_directory}")
